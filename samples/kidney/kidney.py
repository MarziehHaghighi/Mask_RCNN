...
class KidneysDataset(utils.Dataset):
    ...
    def load_Kidneys(self, subjectNamesNormalTrain,isValidationData):
...
#        self.add_class("kidneys", 0, "BG")
        ...
        self.numOfSlices=32;
        idCount=0;
        for i in range(len(subjectNamesNormalTrain)*self.numOfSlices):
            
            subName=self.subjectNamesNormalTrain[int(i/self.numOfSlices)];
            data4D = pickle.load( open( "/fileserver/abd/marzieh/preprocessedData/singleSubjectsV4pc/"+subName+".p","rb" ));
            sliceNum=i % self.numOfSlices;
#            print(subName);
            maxSlice=data4D[subName+'M'].shape[2];
            if maxSlice<self.numOfSlices and sliceNum>=maxSlice:
                m0=data4D[subName+'M'][:,:,maxSlice-1].astype('uint8');
            else:
            ...
            if 1 not in set(m0.flatten()) and 2 not in set(m0.flatten()) :
                ...
                if isValidationData:
                    idCount=+1;
                    self.add_image("kidneys", image_id=idCount, path=None,                
...
#        allData1Slice3D = pickle.load( open( "/home/ch194093/Desktop/kidneydcemri/deepLearningSegmentation/normalData1Slice3D_Pcs.p","rb" ))
        data4D = pickle.load( open( "/fileserver/abd/marzieh/preprocessedData/singleSubjectsV4pc/"+subName+".p","rb" ));
#        print(image_id)
#        sliceNum=image_id % self.numOfSlices;
        maxSlice=data4D[subName+'M'].shape[2];
        if maxSlice<self.numOfSlices and sliceNum>=maxSlice:
            image0=data4D[subName+'D'][:,:,maxSlice-1,0:3];
        else:
            image0=data4D[subName+'D'][:,:,sliceNum,0:3];
