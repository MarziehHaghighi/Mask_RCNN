...
#     clustering_labels = KL.Lambda(lambda x: my_lambda_func(x,pred_class_ids),output_shape=[sh[1],sh[2]], name="lambda_layer")(shared)
    
    clustering_psudo_labels = KL.Lambda(lambda x: my_lambda_func(x,pred_class_ids,gt_ids),output_shape=[sh[1]], name="lambda_layer")(shared)
...
#     print("clustering_labels",clustering_psudo_labels)
    ...
    return clustering_psudo_labels, mrcnn_class_logits, mrcnn_probs, mrcnn_bbox
...
# def mrcnn_cluster_class_loss_graph(target_class_ids, pred_class_logits,
#                            active_class_ids):
#     """Loss for the classifier head of Mask RCNN.
...
#     print("n_c",dataset.num_classes)
    ...
    active_class_ids = np.zeros([dataset.num_classes], dtype=np.int32) #commented by Marzi
...
#                 NMI=KL.Layer(trainable=False, name='nmi', dtype=None)
                ...
                target_class_ids2,mrcnn_class_logits, mrcnn_class, mrcnn_bbox =\
                    fpn_clustering_graph(rois, mrcnn_feature_maps, input_image_meta,
                                         config.POOL_SIZE, config.NUM_CLASSES,gt_ids=target_class_ids,
                                         train_bn=config.TRAIN_BN,
                                     fc_layers_size=config.FPN_CLASSIF_FC_LAYERS_SIZE)
...
#                 sh = K.int_shape(mrcnn_class_logits)
                ...
                NMI = KL.Lambda(lambda_nmi,output_shape=[1], name="lambda_nmi")([target_class_ids,target_class_ids2])
    

#                 print(NMI)
#                 tf.keras.backend.print_tensor(NMI)
