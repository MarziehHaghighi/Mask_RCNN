...
#    print(rois,feature_maps)
#    print(image_shape,pool_size)
#    print(x)
...
def build_fpn_mask_graph2(rois, feature_maps,    #marzieh
                         image_shape, pool_size, num_classes):
    """Builds the computation graph of the mask head of Feature Pyramid Network.
...
#    
#    conv1 = Conv2D(int(xyDim/8), (3,3), activation='relu', padding='same')(conv1)
...
#
##    pred = Conv2D(2, (1, 1),activation='linear',padding='valid')(conv9)
...
#    print('before1',x)
    ...
    x1 = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="mrcnn_mask_conv1")(x)
#    print('before2',x1)
    x2 = KL.TimeDistributed(BatchNorm(axis=3),
                           name='mrcnn_mask_bn1')(x1)
    x2 = KL.Activation('relu')(x2)
    x3 = KL.TimeDistributed(KL.MaxPooling2D(pool_size=(2, 2),strides=2),
                           name="mrcnn_mask_ds1")(x2)   #14 
    
    x3 = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="mrcnn_mask_conv2")(x3)
    x3 = KL.TimeDistributed(BatchNorm(axis=3),
                           name='mrcnn_mask_bn2')(x3)
    x3 = KL.Activation('relu')(x3)
    x4 = KL.TimeDistributed(KL.MaxPooling2D(pool_size=(2, 2),strides=2),
                           name="mrcnn_mask_ds2")(x3)   #7   
    

    x4 = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="mrcnn_mask_conv3")(x4)
    x4 = KL.TimeDistributed(BatchNorm(axis=3),
                           name='mrcnn_mask_bn3')(x4)
    x4 = KL.Activation('relu')(x4)
    
    
    x4 = KL.TimeDistributed(KL.Conv2DTranspose(256, (2, 2), strides=2, activation="relu"),
                           name="mrcnn_mask_deconv1")(x4) #14    
    x5 = KL.concatenate([x4, x3], axis=4)
    x5 = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="mrcnn_mask_conv4")(x5)
    x5 = KL.TimeDistributed(BatchNorm(axis=3),
                           name='mrcnn_mask_bn4')(x5)
    x5 = KL.Activation('relu')(x5)
    
    
    x5 = KL.TimeDistributed(KL.Conv2DTranspose(256, (2, 2), strides=2, activation="relu"),
                           name="mrcnn_mask_deconv2")(x5) #28  
    x6 = KL.concatenate([x5, x2], axis=4)
    x6 = KL.TimeDistributed(KL.Conv2D(256, (3, 3), padding="same"),
                           name="mrcnn_mask_conv5")(x6)
    x6 = KL.TimeDistributed(BatchNorm(axis=3),
                           name='mrcnn_mask_bn5')(x6)
    x6 = KL.Activation('relu')(x6)    
    

    x6 = KL.TimeDistributed(KL.Conv2DTranspose(256, (2, 2), strides=2, activation="relu"),
                           name="mrcnn_mask_deconv3")(x6)
#    x = KL.concatenate([x6, conv2], axis=4)
    x6 = KL.TimeDistributed(KL.Conv2D(num_classes, (1, 1), strides=1, activation="sigmoid"),
                           name="mrcnn_mask")(x6)
#    
#    print('after',x6)
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    # Image meta data
    image_meta = compose_image_meta(image_id, original_shape, window, active_class_ids)
...
#            print('end')
            ...
            if config.head=='def':  # modified by marzieh
                mrcnn_mask = build_fpn_mask_graph(rois, mrcnn_feature_maps,
                                                  config.IMAGE_SHAPE,
                                                  config.MASK_POOL_SIZE,
                                                  config.NUM_CLASSES)
            elif config.head=='unet':
                mrcnn_mask = build_fpn_mask_graph2(rois, mrcnn_feature_maps,
                                                  config.IMAGE_SHAPE,
                                                  config.MASK_POOL_SIZE,
                                                  config.NUM_CLASSES)                
            ...
            if config.head=='def':  # modified by marzieh
                ...
                mrcnn_mask = build_fpn_mask_graph2(detection_boxes, mrcnn_feature_maps,
                                                  config.IMAGE_SHAPE,
                                                  config.MASK_POOL_SIZE,
                                                  config.NUM_CLASSES)
...
#         os.mkdir(self.log_dir);
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        # Callbacks   commented by Marz --> switch to comet_ml
        callbacks = [
...
#             api_key=self.config.cometml_api_key,
#             project_name=self.config.project_name
#         )

#         experiment.set_name(self.config.project_name)
#         experiment.log_others(self.config)
...
#         # CSV Log
#         callback_csv = keras.callbacks.CSVLogger(filename=self.log_dir)
...
#         print(molded_images,image_metas)
